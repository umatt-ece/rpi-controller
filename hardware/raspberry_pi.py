...
class RaspberryPi:
    ...
    def add_spi_device(self, device: SpiDevice, select: str, reset: str = "", shutdown: str = "") -> None:
        ...
        if shutdown:
            self._pinout[self._parse_pin(shutdown)]["pin"].set_direction("output")
            self._pinout[self._parse_pin(shutdown)]["pin"].write(1)
            device.set_shutdown_pin(self._pinout[self._parse_pin(shutdown)]["pin"])
